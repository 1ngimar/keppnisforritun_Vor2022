...
// Öllu stolið af Bergi Snorrasyni kennara af Canvas... basically engu breytt... Hva' er í gangi!?
...
int main()
{
    ...
    rep(i, n - 1)
    {
		scanf("%d", &bosser);
		bosser--;
        g[bosser].push_back(i + 1);
        g[i + 1].push_back(bosser);
    }
	lca_init(g, 0);
    rep(i, m)
    {
		scanf("%d%d", &bosser, &taker);
		bosser--, taker--;
		printf("%s\n", lca(bosser, taker) == taker ? "No" : "Yes");
    }
    return 0;
}